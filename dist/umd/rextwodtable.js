(function (global, factory) {
    ...
    */
    ...
    let RemoveRow = function (table, rowKey) {
    ...
    let LoadCSV = function (table, csvString, config) {
    ...
    let HasColKey = function (table, colKey) {
        return (table.colKeys.indexOf(colKey) !== -1);
    };
    let HasKey = function (table, rowKey, colKey) {
        return HasRowKey(table, rowKey) && HasColKey(table, colKey);
    };

    let ConvertRow = function (table, rowKey, callback = TypeConvert, scope) {
    ...
    let EachCol = function (table, callback, scope) {
    ...
    let Set = function (table, rowKey, colKey, value) {
    ...
    let SortRow = function (table, callback, scope) {
        let sortCallback;
    ...
    let SortCol = function (table, callback, scope) {
        let sortCallback;
        if (typeof (callback) === 'function') {
            if (scope) {
                sortCallback = callback.bind(scope);
    ...
    let NextColKey = function (table, colKey, step = 1, wrap = true) {
    ...
    let PreviousColKey = function (table, colKey, step = 1, wrap = true) {
        return NextColKey(table, colKey, -step, wrap);
    };
    let NextRowKey = function (table, rowKey, step = 1, wrap = true) {
    ...
    let PreviousRowKey = function (table, rowKey, step = 1, wrap = true) {
        return NextRowKey(table, rowKey, -step, wrap);
    };

    let IsValueInRow = function (table, rowKey, value) {
        if (!HasRowKey(table, rowKey)) {
            return false;
        }
        let found = false;
